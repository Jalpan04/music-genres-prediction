...
def main():
    print("=== Music Genre Classification: Hybrid (ResNet + MLP) Fusion ===")
    ...
    if not os.path.exists(features_csv):
        print(f"Error: {features_csv} not found. Run main.py or feature extraction first.")
        return
    ...
    # Using same random_state=42 ensures consistency with previous experiments
    ...
    # 5. Prepare Feature DataFrames for Datasets
    # We need to scale features based on TRAIN set only
    train_filenames = set(os.path.basename(p) for p in paths_train)
    test_filenames = set(os.path.basename(p) for p in paths_test)
    
    # Split dataframe
    # Note: df might contain files not in audio_paths if re-downloaded, so strict filtering
    ...
    # Lower LR for fine-tuning, but slightly higher than pure ResNet maybe?
    ...
    for epoch in range(epochs):
        ...
        if val_acc > best_acc:
            best_acc = val_acc
            torch.save(model.state_dict(), 'results/best_hybrid_model.pth')
            
    print(f"Best Segment Accuracy: {best_acc:.4f}")
    
    # 9. Voting Eval
    print("\nCalculating Song-Level Accuracy (Majority Voting)...")
    model.load_state_dict(torch.load('results/best_hybrid_model.pth'))
    model.eval()
    
    song_total = len(paths_test)
    segments_per_song = 10
    
    # Manual loop similar to segmented
    all_preds = []
    all_targets = []
    ...
    if len(all_preds) == song_total * segments_per_song:
        ...
        print(f"Hybrid Voting Accuracy: {acc_dict['accuracy']*100:.2f}%")
        
        plot_confusion_matrix(final_song_targets, final_song_preds, classes, title='Hybrid Voting Confusion Matrix', filename='hybrid_voting_cm.png')
    else:
        print("Warning: Prediction variance (length mismatch). Skipping voting.")
        
    plot_training_history(history, filename='hybrid_training_history.png')
